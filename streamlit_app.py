...
def clean_folder_id(folder_id):
    if not folder_id:
        return ""
    # 去除結尾句號、問號參數及 URL 前綴
    cleaned = str(folder_id).strip().rstrip('.')
    if '/' in cleaned:
...
def update_vendor_excel_on_drive(vendor_data, folder_id):
    ...
    try:
        if items:
            ...
            updated_df = pd.concat([existing_df, new_df], ignore_ignore_index=True) if hasattr(pd, 'concat') else existing_df.append(new_df, ignore_index=True)
