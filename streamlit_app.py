...
def generate_content_with_retry(bytes_data, mime_type, prompt):
    """具備自動輪換模型與 Rate Limit 退避重試機制的生成函數"""
    ...
    for model_name in candidate_models:
        try:
            ...
            err_str = str(e)
            last_error = e
            # 若觸發 429 Rate Limit / Quota Exceeded，稍微等待後嘗試下一個候選模型
            if "429" in err_str or "Quota exceeded" in err_str or "LIMIT_EXCEEDED" in err_str:
                time.sleep(2)
                continue
            else:
                # 若為其他錯誤也嘗試下一個模型
    ...
    raise last_error if last_error else Exception("所有可用 Gemini 模型皆達到頻率限制或不可用，請稍後再試。")
        ...
        if file_key not in st.session_state:
            with st.spinner("AI 正在辨識文件、服務範疇與匯款資訊..."):
                try:
                    ...
                    res_text = generate_content_with_retry(bytes_data, mime_type, prompt)
